...
class TlShapeUtil extends ShapeUtil<TlShape> {
  ...
  override component(shape: TlShape) {
    ...
    const arrowPaths = arrowVertices
      .map((_, i) => {
        if (i % 2 === 0) {
          // Start of each arrow segment
      ...
      .map((point, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={subheadingX} y={descriptionY} textAnchor="middle">
              Description {i + 1}
            </text>
          </g>
        );
...
function getTlVertices(shape: TlShape): { tl: Vec[]; arrow: Vec[] } {
  ...
  for (let i = 1; i <= parts; i++) {
    ...
    const direction = i % 2 === 0 ? -50 : 50; // Direction of the arrow (Up/Down)
    arrow.push(new Vec(x, 200)); // Start point of the arrow (intersection with the main line)
    arrow.push(new Vec(x, 200 + direction)); // End point of the arrow
  }
  return { tl, arrow };
}
    ...
    <div style={{ position: "fixed", width: "100vw", height: "50vh" }}>
      <Tldraw
        ...
        onMount={(editor) => {
          ...
          editor.sideEffects.registerBeforeChangeHandler( // So that tl cannot be changed
            "shape",
            (prev, next) => {
              if (
